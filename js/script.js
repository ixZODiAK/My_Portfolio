...
let certificates = JSON.parse(localStorage.getItem('certificates')) || [
    { id: 1, name: "OSCP", issuer: "Offensive Security", date: "2024", link: "https://www.offsec.com", description: "شهادة OSCP المهنية - اختبار الاختراق العملي" },
    { id: 2, name: "CEH Master", issuer: "EC-Council", date: "2024", link: "https://www.eccouncil.org", description: "شهادة الهاكر الأخلاقي المعتمد" },
...
let projects = JSON.parse(localStorage.getItem('projects')) || [
    { id: 1, name: "Vulnerable Web Lab", category: "Web Security", date: "2024", description: "مختبر ويب ضعيف لتعليم ثغرات OWASP Top 10", technologies: "JavaScript,Node.js,Express", github: "https://github.com/ixZODiAK/vulnerable-lab" },
    { id: 2, name: "Auto Pentest Toolkit", category: "Automation", date: "2024", description: "أدوات آلية لأتمتة اختبارات الاختراق", technologies: "Python,Bash,Nmap,Metasploit", github: "https://github.com/ixZODiAK/auto-pentest" }
];

// ========== بيانات رايت اب الماكينات ==========
let machineWriteups = JSON.parse(localStorage.getItem('machineWriteups')) || [
    { id: 1, name: "Lame", platform: "HackTheBox", difficulty: "easy", date: "2024-01-15", description: "اختراق جهاز Lame باستخدام ثغرة Samba", tags: "Samba,Linux,CVE-2007-2447", commands: "nmap -sV -sC 10.10.10.3\nsearchsploit samba 3.0.20\nmsfconsole -q -x 'use exploit/multi/samba/usermap_script; set RHOST 10.10.10.3; exploit'", walkthrough: "الشرح الكامل لاختراق جهاز Lame...", likes: 42, views: 245 },
    { id: 2, name: "Blue", platform: "HackTheBox", difficulty: "easy", date: "2024-01-10", description: "اختراق جهاز Blue باستخدام ثغرة EternalBlue", tags: "Windows,EternalBlue,MS17-010", commands: "nmap -p445 --script smb-vuln-ms17-010 10.10.10.40\nmsfconsole -q -x 'use exploit/windows/smb/ms17_010_eternalblue; set RHOST 10.10.10.40; exploit'", walkthrough: "الشرح الكامل لاختراق Blue...", likes: 67, views: 412 }
];

// ========== بيانات إحصائيات المنصات ==========
let platformStats = JSON.parse(localStorage.getItem('platformStats')) || {
    hackthebox: 45,
    tryhackme: 62,
    portswigger: 38,
    vulnhub: 25,
...
const translations = {
    ar: {
        'nav-home': 'الرئيسية', 'nav-cv': 'السيرة', 'nav-certs': 'الشهادات', 'nav-projects': 'المشاريع',
        'nav-writeups': 'الرايت اب', 'nav-vuln': 'الثغرات', 'nav-blog': 'المدونة', 'nav-contact': 'تواصل',
        ...
        'tab-delete': '🗑️ حذف مقال', 'tab-certs': '🏆 شهادات', 'tab-projects': '📁 مشاريع',
        'tab-machines': '🖥️ رايت اب', 'tab-stats': '📊 إحصائيات',
        'add-title': 'إضافة مقال جديد', 'edit-title': 'تعديل مقال', 'delete-title': 'حذف مقال',
        'post-title': 'عنوان المقال', 'post-content': 'محتوى المقال كاملاً', 'post-tags': 'الوسوم (مفصولة بفواصل)',
        'publish': '📝 نشر المقال', 'update': '🔄 تحديث المقال', 'delete': '❌ حذف نهائي',
        'select-edit': 'اختر مقال للتعديل', 'select-delete': 'اختر مقال للحذف',
        'new-title': 'العنوان الجديد', 'new-content': 'المحتوى الجديد', 'add-media': '➕ إضافة صور/فيديوهات',
        'upload-media': 'رفع ملفات', 'hero-name': 'HAMMAD ELSAYED - Pentester',
        'hero-command': '#!/bin/bash - "Hack The Planet"',
        'hero-desc': 'مختبر اختراق | OSCP | CEH | CTF Player',
        'skill-web': 'اختبار الاختراق للويب', 'skill-web-desc': 'SQLi, XSS, CSRF, SSRF',
        'skill-network': 'أمن الشبكات', 'skill-network-desc': 'Nmap, Metasploit, Wireshark',
        'skill-reverse': 'الهندسة العكسية', 'skill-reverse-desc': 'Ghidra, IDA, x86/x64',
        'skill-crypto': 'التشفير', 'skill-crypto-desc': 'RSA, AES, Hashing',
        'cv-title': '┌─[CV]─[السيرة الذاتية]', 'cv-name': 'HAMMAD ELSAYED',
        'cv-job': 'Penetration Tester', 'cv-download': '📥 تحميل CV',
        'cv-summary': 'ملخص احترافي', 'cv-summary-text': 'باحث أمني ومختبر اختراق، مؤسس فريق BLACK V الأمني.',
        'cv-experience': 'الخبرات المهنية',
        'cv-exp1-title': 'Cybersecurity Researcher', 'cv-exp1-company': 'BLACK V Team',
        'cv-exp1-desc1': 'اختبار اختراق تطبيقات الويب', 'cv-exp1-desc2': 'تطوير أدوات أتمتة',
        'cv-exp1-desc3': 'توثيق التقارير الفنية',
        'cv-exp2-title': 'Pentester & Developer', 'cv-exp2-company': 'Front-End, Python',
        'cv-exp2-desc1': 'تطوير مختبر ويب ضعيف', 'cv-exp2-desc2': 'إدارة بيئات افتراضية',
        'cv-exp2-desc3': 'بحث في تشفير الهاردوير',
        'cv-education': 'التعليم', 'cv-edu-title': 'جامعة الازهر', 'cv-edu-major': 'كلية الشريعة والقانون',
        'cv-skills-title': 'المهارات التقنية', 'cv-languages': 'اللغات',
        'lang-arabic': 'العربية', 'lang-native': 'اللغة الأم', 'lang-english': 'الإنجليزية',
        'lang-fluent': 'متوسط (B1)', 'cv-location': 'مصر، الغربية',
        ...
        'blog-title': '┌─[BLOG]─[كتابات CTF]', 'read-more': '📖 اقرأ المزيد',
        'contact-title': '┌─[CONTACT]─[تواصل معي]', 'send-message': 'أرسل رسالة',
        'your-name': 'اسمك', 'your-email': 'بريدك', 'your-message': 'رسالتك',
        'send': 'إرسال', 'sending': 'جاري الإرسال...',
        'discord': 'Discord: zodiakx_x', 'twitter': 'Twitter: zodiakx_x',
        'github': 'GitHub: ixZODiAK', 'footer': '© 2026 HAMMAD ELSAYED - مختبر اختراق',
        'footer-quote': '#HackThePlanet',
        'machine-writeups-title': '┌─[MACHINES]─[رايت اب الماكينات]',
        'stats-solved': 'ماكينة محلولة'
    },
    en: {
        'nav-home': 'Home', 'nav-cv': 'CV', 'nav-certs': 'Certifications', 'nav-projects': 'Projects',
        'nav-writeups': 'Writeups', 'nav-vuln': 'Vulnerabilities', 'nav-blog': 'Blog', 'nav-contact': 'Contact',
        ...
        'tab-delete': '🗑️ Delete Post', 'tab-certs': '🏆 Certifications', 'tab-projects': '📁 Projects',
        'tab-machines': '🖥️ Writeups', 'tab-stats': '📊 Stats',
        'add-title': 'Add New Writeup', 'edit-title': 'Edit Writeup', 'delete-title': 'Delete Writeup',
        'post-title': 'Post Title', 'post-content': 'Full Writeup Content', 'post-tags': 'Tags (comma separated)',
        'publish': '📝 Publish', 'update': '🔄 Update', 'delete': '❌ Delete',
        'select-edit': 'Select writeup to edit', 'select-delete': 'Select writeup to delete',
        'new-title': 'New title', 'new-content': 'New content', 'add-media': '➕ Add Images/Videos',
        'upload-media': 'Upload Files', 'hero-name': 'HAMMAD ELSAYED - Pentester',
        'hero-command': '#!/bin/bash - "Hack The Planet"',
        'hero-desc': 'Pentester | OSCP | CEH | CTF Player',
        'skill-web': 'Web Penetration', 'skill-web-desc': 'SQLi, XSS, CSRF, SSRF',
        'skill-network': 'Network Security', 'skill-network-desc': 'Nmap, Metasploit, Wireshark',
        'skill-reverse': 'Reverse Engineering', 'skill-reverse-desc': 'Ghidra, IDA, x86/x64',
        'skill-crypto': 'Cryptography', 'skill-crypto-desc': 'RSA, AES, Hashing',
        'cv-title': '┌─[CV]─[Resume]', 'cv-name': 'HAMMAD ELSAYED',
        'cv-job': 'Penetration Tester', 'cv-download': '📥 Download CV',
        'cv-summary': 'Professional Summary', 'cv-summary-text': 'Cybersecurity Researcher, Founder of BLACK V Team.',
        'cv-experience': 'Experience',
        'cv-exp1-title': 'Cybersecurity Researcher', 'cv-exp1-company': 'BLACK V Team',
        'cv-exp1-desc1': 'Web penetration testing', 'cv-exp1-desc2': 'Automation tools development',
        'cv-exp1-desc3': 'Technical reports writing',
        'cv-exp2-title': 'Pentester & Developer', 'cv-exp2-company': 'Front-End, Python',
        'cv-exp2-desc1': 'Vulnerable web lab', 'cv-exp2-desc2': 'Virtualization management',
        'cv-exp2-desc3': 'Hardware crypto research',
        'cv-education': 'Education', 'cv-edu-title': 'Al-Azhar University', 'cv-edu-major': 'Law School',
        'cv-skills-title': 'Technical Skills', 'cv-languages': 'Languages',
        'lang-arabic': 'Arabic', 'lang-native': 'Native', 'lang-english': 'English',
        'lang-fluent': 'Intermediate (B1)', 'cv-location': 'Egypt, Gharbia',
        'vuln-title': '┌─[TOP 10]─[Vulnerabilities]',
        'vuln-desc-sql': 'SQL Injection - Attacker executes malicious SQL commands',
        'vuln-prevention-sql': '✅ Use Prepared Statements',
        'vuln-desc-xss': 'Inject malicious scripts into web pages',
        'vuln-prevention-xss': '✅ Output encoding - CSP',
        'vuln-desc-csrf': 'Cross-Site Request Forgery',
        'vuln-prevention-csrf': '✅ CSRF Tokens - SameSite Cookies',
        'vuln-desc-lfi': 'Read sensitive files from server',
        ...
        'vuln-desc-cmd': 'Execute OS commands',
        'vuln-prevention-cmd': '✅ escapeshellcmd()',
        'vuln-desc-idor': 'Unauthorized access to objects',
        'vuln-prevention-idor': '✅ Check permissions',
        'vuln-desc-xxe': 'XML External Entity attack',
        'vuln-prevention-xxe': '✅ Disable XXE',
        'vuln-desc-ssrf': 'Server-Side Request Forgery',
        'vuln-prevention-ssrf': '✅ URL whitelist',
        'vuln-desc-auth': 'Broken Authentication',
        'vuln-prevention-auth': '✅ MFA - Strong passwords',
        ...
        'discord': 'Discord: zodiakx_x', 'twitter': 'Twitter: zodiakx_x',
        'github': 'GitHub: ixZODiAK', 'footer': '© 2026 HAMMAD ELSAYED - Pentester',
        'footer-quote': '#HackThePlanet',
        'machine-writeups-title': '┌─[MACHINES]─[CTF Writeups]',
        'stats-solved': 'Machines Solved'
    }
};
...
function adminLogin() {
    if (document.getElementById('adminPass').value === ADMIN_PASSWORD) {
        ...
        updateEditSelect();
        updateDeleteSelect();
        // تحديث عرض الجداول في لوحة الأدمن
        displayCertificatesAdmin();
        displayProjectsAdmin();
        displayMachinesAdmin();
        loadPlatformStatsEditor();
        alert(currentLang === 'ar' ? 'مرحباً بك أيها الأدمن!' : 'Welcome Admin!');
    } else {
...
function switchAdminTab(tab) {
    ...
    if (tab === 'certs') {
        document.getElementById('certsForm').classList.add('active');
        displayCertificatesAdmin();
    }
    if (tab === 'projects') {
        document.getElementById('projectsForm').classList.add('active');
        displayProjectsAdmin();
    }
    if (tab === 'machines') {
        document.getElementById('machinesForm').classList.add('active');
        displayMachinesAdmin();
    }
    if (tab === 'stats') {
        document.getElementById('statsForm').classList.add('active');
        loadPlatformStatsEditor();
    }
}
...
// ========== دوال جلب وحفظ المقالات من السحابة ==========
...
function displayCertificatesPublic() {
    ...
    certificates.forEach(cert => {
        html += `<div class="cert-card" onclick="window.open('${cert.link || '#'}', '_blank')">
            <div style="height: 180px; background: linear-gradient(135deg, var(--accent-cyan), var(--accent-purple)); display: flex; align-items: center; justify-content: center; font-size: 3rem;">🏆</div>
...
function displayCertificatesAdmin() {
    ...
    let html = '';
    certificates.forEach(cert => {
        html += `<div style="background:rgba(15,20,25,0.8);border-radius:15px;padding:15px;margin-bottom:15px;border:1px solid var(--accent-purple);">
            <div style="display:flex;justify-content:space-between;align-items:center;"><div><strong style="color:var(--accent-cyan);">${cert.name}</strong><br><small>${cert.issuer} | ${cert.date}</small></div>
            <div><button class="delete-btn" onclick="deleteCertificate(${cert.id})" style="background:var(--accent-red);color:white;border:none;padding:5px 12px;border-radius:8px;cursor:pointer;">🗑️ حذف</button></div></div>
            <p style="margin-top:10px;">${cert.description?.substring(0, 100) || ''}...</p></div>`;
    });
    container.innerHTML = html;
}
...
function displayProjectsPublic() {
    ...
    projects.forEach(project => {
        html += `<div class="project-card" onclick="window.open('${project.github || '#'}', '_blank')">
            <div style="height: 180px; background: linear-gradient(135deg, var(--accent-cyan), var(--accent-purple)); display: flex; align-items: center; justify-content: center; font-size: 3rem;">📁</div>
            <div class="project-info"><div class="project-name">${project.name}</div><div class="project-category">📂 ${project.category} | 📅 ${project.date}</div>
            <p>${project.description?.substring(0, 80) || ''}...</p>
            <div style="display:flex;flex-wrap:wrap;gap:5px;">${(project.technologies || '').split(',').map(t => `<span class="admin-badge">${t.trim()}</span>`).join('')}</div></div></div>`;
    });
    container.innerHTML = html;
}
...
function displayProjectsAdmin() {
    ...
    projects.forEach(proj => {
        html += `<div style="background:rgba(15,20,25,0.8);border-radius:15px;padding:15px;margin-bottom:15px;border:1px solid var(--accent-green);">
            <div style="display:flex;justify-content:space-between;"><div><strong style="color:var(--accent-cyan);">${proj.name}</strong><br><small>${proj.category}</small></div>
            <div><button class="delete-btn" onclick="deleteProject(${proj.id})" style="background:var(--accent-red);color:white;border:none;padding:5px 12px;border-radius:8px;cursor:pointer;">🗑️ حذف</button></div></div>
            <p>${proj.description?.substring(0, 100) || ''}...</p></div>`;
    });
    container.innerHTML = html;
}
...
function displayMachineWriteups() {
    ...
    machineWriteups.forEach(m => {
        ...
        html += `<div class="writeup-machine-card" onclick="openMachineModal(${m.id})">
            <div class="writeup-machine-header"><span class="writeup-machine-name">${m.name}</span>
            <div><span class="writeup-machine-platform">${m.platform}</span><span class="writeup-machine-difficulty difficulty-${m.difficulty}">${diffText}</span></div></div>
            <div class="writeup-machine-date">📅 ${m.date}</div>
            <div class="writeup-machine-desc">${m.description?.substring(0, 100) || ''}...</div>
...
function displayMachinesAdmin() {
    ...
    machineWriteups.forEach(m => {
        html += `<div style="background:rgba(15,20,25,0.8);border-radius:15px;padding:15px;margin-bottom:15px;border:1px solid var(--accent-green);">
            <div style="display:flex;justify-content:space-between;"><div><strong style="color:var(--accent-cyan);">${m.name}</strong><br><small>${m.platform}</small></div>
            <div><button class="delete-btn" onclick="deleteMachine(${m.id})" style="background:var(--accent-red);color:white;border:none;padding:5px 12px;border-radius:8px;cursor:pointer;">🗑️ حذف</button></div></div>
            <p>${m.description?.substring(0, 100) || ''}...</p></div>`;
    });
    container.innerHTML = html;
}
...
function openMachineModal(id) {
    ...
    const content = `<div><div style="display:flex;gap:10px;margin-bottom:15px;"><span class="writeup-machine-platform">${m.platform}</span><span>📅 ${m.date}</span></div>
        <div class="writeup-machine-tags">${(m.tags || '').split(',').map(t => `<span class="writeup-machine-tag">#${t.trim()}</span>`).join('')}</div>
        ${m.commands ? `<div style="background:#0a0c0f;padding:15px;border-radius:10px;margin:15px 0;"><h4 style="color:var(--accent-yellow);">💻 الأوامر</h4>${m.commands.split('\n').map(c => `<div style="font-family:monospace;">$ ${c}</div>`).join('')}</div>` : ''}
        <div style="background:rgba(0,255,0,0.05);padding:20px;border-radius:15px;"><h3 style="color:var(--accent-green);">📝 الشرح الكامل</h3><p style="line-height:1.8;">${m.walkthrough?.replace(/\n/g, '<br>') || ''}</p></div></div>`;
...
function displayPlatformStats() {
    ...
    let html = '';
    platforms.forEach(p => { html += `<div class="stat-card"><div class="stat-platform-icon">${p.icon}</div><div class="stat-platform-name">${p.name}</div><div class="stat-count">${platformStats[p.id] || 0}</div><div class="stat-label">${currentLang === 'ar' ? 'ماكينة محلولة' : 'Machines Solved'}</div></div>`; });
    const total = Object.values(platformStats).reduce((a, b) => a + b, 0);
    html += `<div class="stat-card"><div class="stat-platform-icon">📊</div><div class="stat-platform-name">Total</div><div class="stat-count">${total}</div><div class="stat-label">${currentLang === 'ar' ? 'إجمالي الماكينات' : 'Total Machines'}</div></div>`;
    container.innerHTML = html;
}
...
(function() { if (typeof emailjs !== 'undefined') emailjs.init(EMAILJS_CONFIG.publicKey); })();
...
function sendEmail(event) {
    ...
    if (typeof emailjs === 'undefined') {
        ...
        formMessage.innerHTML = '⚠️ EmailJS not loaded';
        sendBtn.disabled = false;
        return;
    }
    emailjs.send(EMAILJS_CONFIG.serviceID, EMAILJS_CONFIG.templateID, { from_name: fromName, from_email: fromEmail, message: message, to_email: 'zodiak0x6@gmail.com' })
        .then(() => {
            formMessage.style.display = 'block';
            formMessage.style.background = 'rgba(0,255,0,0.1)';
            formMessage.style.border = '1px solid #0f0';
            formMessage.style.color = '#0f0';
            formMessage.innerHTML = currentLang === 'ar' ? '✅ تم إرسال الرسالة بنجاح!' : '✅ Message sent!';
        ...
        .catch(() => {
            ...
            formMessage.innerHTML = currentLang === 'ar' ? '❌ فشل الإرسال' : '❌ Failed to send';
            setTimeout(() => formMessage.style.display = 'none', 5000);
        })
...
window.onclick = function(event) {
    const fullModal = document.getElementById('fullPostModal');
    if (event.target === fullModal) closeFullPostModal();
    const globalModal = document.getElementById('globalModal');
    if (event.target === globalModal) globalModal.classList.remove('active');
